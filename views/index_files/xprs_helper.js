...
XPRSHelper.presetTypes = {
		...
		"PRICE":{"TYPE":"PRICE","NAME":"PRICE","GROUP":"UNRESOLVED","COLOR":"#64A764","PAGE":0}, // ECOMMERCE
...
XPRSHelper.getXprsCookie = function(cookieName){
	...
	try {
	    ...
	    for(var i = 0; i <ca.length; i++) {
	        ...
	        if (c.indexOf(name) == 0) {
	        	console.log("got " + c.substring(name.length,c.length) )
...
XPRSHelper.POST = function(postPath, params, callbackFunc,responseType){
	$.ajax({
		  type: "POST",
		  url: XPRSHelper.getServerPath() + postPath,
		  data: params,
...
XPRSHelper.SAFEPOST = function(url,params,saveKey,saveName,callbackFunc){
	if (XPRSHelper.pendingActionExists(saveKey) || XPRSHelper.futureQueueSize(saveKey) > 0){
		if (XPRSHelper.pendingActionExists(saveKey)){
			//console.log("found pending actions for key " + saveKey);
		}else if (XPRSHelper.futureQueueSize(saveKey) > 0){
			//console.log("future queue is still not empty for key " + saveKey + " size is " + XPRSHelper.futureQueueSize(saveKey) +  " " + JSON.stringify(XPRSHelper.saveQueue["FUTURE"][saveKey]));
		}
	...
	tooltTips = [
		     		{
			     		...
			     		"selector":"#add-stripe",
			     		"container_selector":".control-handle:eq(1)",
			     		"frame":"viewer",
...
//		     		},
	...
	//if (tipObj.frame != "dual"){
		//tooltipRefrenceContainer = $(".master.item-box").not(".header-box").first();
	//}
...
XPRSHelper.invokeLogin = function(callbackFunc,form){
	...
	if (currentUser == "guest" || nextUrl != "" || forceDialog){
		 var login_css = $("<link>");
		 login_css.attr({ 
		      rel:  "stylesheet",
		      type: "text/css",
		      href: XPRSHelper.getServerPath() + "/css/login.css"
		    });
		 $("head").append(login_css);
		 
		 
		 $.ajax({
			  url: XPRSHelper.getServerPath() + "/js/login.js?v=2",
			  dataType: 'script',
			  success: function(){
				  setTimeout(function(){
